...
public class GroupController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if ("admin".equals(tab)) {
                session.setAttribute("LIST", dao.getListByRoleID(1));
                request.setAttribute("GROUP", "admin");
            } else if ("member".equals(tab)) {
                session.setAttribute("LIST", dao.getListByRoleID(2));
                request.setAttribute("GROUP", "member");
            } else {
                session.setAttribute("LIST", dao.getListAccount());

...
public class DetailController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ArrayList<DetailDTO> list = (ArrayList<DetailDTO>) session.getAttribute("TMP");
            ArrayList<DetailDTO> tmp = new ArrayList<>();
            for (DetailDTO detailDTO : list) {
                if(detailDTO.getPro().getProID() == Integer.parseInt(proid)){
                    tmp.add(detailDTO);
                }
            }
            session.setAttribute("DETAIL", tmp);
